...
objs = [obj2, obj]
print(objs)
objs.sort(key=lambda x: x.sort_key())
print(objs)
...
* 输入参数可以有多个，可以接受不定参数如：*args或者 **kwargs
...
* 可选参数为指明原函数的哪些属性要直接被赋值给wrapper函数的匹配属性的元组，并且这些wrapper函数的属性将使用原函数的对应属性来更新
* 为了允许出于内省和其他目的访问原始函数（例如绕过 [`lru_cache()`](https://docs.python.org/zh-cn/3.9/library/functools.html?highlight=update_wrapper#functools.lru_cache) 之类的缓存装饰器），此函数会自动为 wrapper 添加一个指向被包装函数的 `__wrapped__` 属性。
...
if __name__ == '__main__':
    run = wrap2(test)	# flask的做法是用列表保存对象，等到需要时调用对象列表.EE=]
    run(1)
```

# 11.sys.modules

1. sys.modules是一个全局字典，从python程序启动到加载到内存，用于保存当前已导入(加载的所有模块名和模块对象)

2. 在python的模块查找中，sys.modules起到缓存作用，避免了模块的重复加载
