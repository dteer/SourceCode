...
* 一条需要url匹配过程
  ...
  * 路由规则：/all/page/\<int(signed=True):page\>

  1. url_map = Map()

  2. urls = url_map.bind()

  3. urls.match(path_info="/all/page/1")
     ...
     * 参数
       ...
       * ```python
         ...
         try:
             # Rule-match	
         	# 检查...并生成url的变量字典：rv={"page":1}
         	rv = rule.match(path, method)	# 重点
         ...
...
* 代码
  ...
  * ```python
    ...
    # 路由之间按照下面变量排序
    return (
        bool(self.arguments),	# 是否有url变量
        -len(self._static_weights),	# 有多少个静态信息
        self._static_weights,	# 静态信息的索引内容
        -len(self._argument_weights),	# url变量个数
        self._argument_weights,	# url变量的权重内容列表
    )
    # return = (True, -2, [(0, -3), (0, -4)], -1, [50])
    ...
    return return 1 if self.alias else 0, -len(self.arguments), -len(self.defaults or ())
